...
copy(
  R("docs/internal/backend/stats.backend.json"),
  path.join(DATA, "backend/stats.json"),
);
tomlToJson(
  R("docs/internal/backend/modules.manifest.toml"),
  path.join(DATA, "backend/modules.json"),
);
tomlToJson(
  R("docs/internal/backend/tree.annotations.toml"),
  path.join(DATA, "backend/tree.json"),
);

// frontend: already JSON.
copy(
  R("docs/internal/frontend/stats.frontend.json"),
  path.join(DATA, "frontend/stats.json"),
);
copy(
  R("docs/internal/frontend/modules.manifest.json"),
  path.join(DATA, "frontend/modules.json"),
);
copy(
  R("docs/internal/frontend/tree.annotations.json"),
  path.join(DATA, "frontend/tree.json"),
);
...
if (existsSync(shotsDir)) {
  ...
  for (const f of files) {
    ...
    if (!scenes.has(scene))
      scenes.set(scene, { scene, light: null, dark: null });
...
const topics =
  parseToml(readFileSync(R("tooling/product-features.toml"), "utf8")).topics ??
  {};
...
// The manual — the whole guide on one page (its prose IS the product topics it
// concatenates). In the index so EVERY consumer's search (this site, aevum-web /help,
// marketing) can surface it as a destination, not only the topics inside it.
index.push({
  slug: "manual",
  url: "/manual",
  title: "User manual",
  section: "Manual",
  summary:
    "The complete guide on one page — every product topic in order, with a PDF to download or print.",
  headings: [],
});
