...
function render(results: { entry: Entry; heading?: Heading }[], panel: HTMLElement, input: HTMLInputElement) {
  if (!results.length) {
    panel.innerHTML =
    ...
    panel.innerHTML = results
      ...
      .map(({ entry, heading }) => {
        ...
        const tag =
          entry.section === 'Engineering'
            ? '<span class="ml-2 shrink-0 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-slate-500 dark:bg-slate-800 dark:text-slate-400">Eng</span>'
            : '';
